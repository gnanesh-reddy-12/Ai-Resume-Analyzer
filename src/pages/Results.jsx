...
function SectionCard({ title, children, style }) {
  return (
    <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={spring}
      className="ek-card results-section-card"
      style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--r-xl)", overflow: "hidden", ...style }}>
      {title && (
        <div className="results-section-header" style={{ padding: "16px 22px", borderBottom: "1px solid var(--border)", background: "var(--surface-2)" }}>
          <h2 style={{ fontSize: 14, fontWeight: 700, color: "var(--text-1)" }}>{title}</h2>
        </div>
      )}
      <div className="results-section-body" style={{ padding: "20px 22px" }}>{children}</div>
    </motion.div>
  )
}
...
export default function Results() {
  ...
  useEffect(() => {
    ...
    const fetchCoreData = async () => {
      if (isHistoryMode) {
        ...
        supabase.from("analyses").select("*").eq("id", id).single()
          .then(({ data, error }) => {
            if (error || !data) throw new Error("Failed to load analysis");
            setCoreData(data)
            if (data.improvement_suggestions) {
              const suggs = data.improvement_suggestions.suggestions || data.improvement_suggestions;
              if (suggs && Object.keys(suggs).length > 0) setImproveData(suggs);
              if (data.improvement_suggestions.cover_letter_text) setCoverLetter(data.improvement_suggestions.cover_letter_text);
              if (data.improvement_suggestions.interview_questions) setInterviewData(data.improvement_suggestions.interview_questions);
            }
          })
    ...
    fetchCoreData()
  }, [id, isHistoryMode, location.state, resumeFile, token, navigate, jobDescription, company, role])

  const handleAiImprove = async () => {
    if (!isHistoryMode && (!resumeFile || !jobDescription)) return
  ...
  const handleMockInterview = async () => {
    if (!isHistoryMode && !jobDescription) return
  ...
  return (
    <AppLayout activeId={isHistoryMode ? "history" : "dashboard"}>
      ...
      <div className="container" style={{ paddingTop: 0, paddingBottom: 96 }}>
        ...
        {/* Page title — only in live mode */}
        {!isHistoryMode && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={spring} style={{ marginBottom: 20 }}>
        ...
        {isHistoryMode && coreData && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={spring} style={{ marginBottom: 20 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
              ...
              <span className="history-badge" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
            ...
            <h1 style={{ fontSize: "clamp(22px,4vw,30px)", fontWeight: 800, letterSpacing: "-0.04em", marginBottom: 5 }}>
              {coreData.company_name ? `${coreData.company_name} — ATS Report` : "ATS Report"}
            </h1>
            <p style={{ fontSize: 13.5, color: "var(--text-3)" }}>
              {coreData.job_role ? <>{coreData.job_role} · </> : null}{coreData.filename}
            </p>
          </motion.div>
        )}

        {/* ── UNIFIED GLASS NAV BAR ── */}
        <div className="results-glass-nav">
          {/* Tab buttons */}
          <div className="results-glass-tabs" style={{ margin: "0 auto" }}>
            {tabs.map((t, i) => (
              <button
                key={i}
                onClick={() => setActiveTab(i)}
                className={activeTab === i ? "olive-tab-active" : "olive-tab"}
        ...
        >
          <div className="res-grid-2-1">
            <SectionCard style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 0, minWidth: 180, height: "100%" }}>
          ...
          <SectionCard title="Job Description Analysis">
            {jobAnalysis && (
              <div className="res-grid-2" style={{ gap: 12, marginBottom: 20 }}>
                {[
          ...
          {loading.improve ? (
            ...
            <SectionCard>
              <div style={{ textAlign: "center", padding: "20px 0" }}>
                ...
                <button onClick={handleAiImprove} disabled={!isHistoryMode && !resumeFile} className="btn-accent" style={{ padding: "10px 24px" }}>
                  {(isHistoryMode || resumeFile) ? "Generate AI Improvements" : "Upload resume to enable AI Improvements"}
                </button>
              </div>
            </SectionCard>
          ) : (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                ...
                {improveData.skills_recommendation && (
                  <div>
                    ...
                    <div className="res-grid-2" style={{ gap: 10 }}>
                        ...
                        <div key={i}>
                          ...
                          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                            {sect.bullets?.map((b, bi) => (
                              <div key={bi} className="res-bullet-grid" style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: "var(--r-md)", padding: "14px 16px" }}>
                                <div>
                                ...
                                <div className="res-bullet-divider">
            ...
            {displayCoverLetter ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                  ...
                  <button onClick={handleCoverLetter} disabled={loading.cover || (!isHistoryMode && !resumeFile)} className="btn-accent" style={{ padding: "10px 24px" }}>
                    Generate Cover Letter
                  </button>
                </div>
                {!isHistoryMode && !resumeFile && <p style={{ fontSize: 12, color: "var(--danger)" }}>Requires resume file to generate.</p>}
        ...
        <div style={{ display: activeTab === 3 ? "flex" : "none", flexDirection: "column", gap: 16 }}>
          <SectionCard title="Mock Interview Generator">
            {displayInterview ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                  <button onClick={handleMockInterview} disabled={loading.interview || (!isHistoryMode && !jobDescription)} className="btn-accent" style={{ padding: "10px 24px" }}>
                    Generate Mock Interview
                  </button>
                </div>
                {!isHistoryMode && !jobDescription && <p style={{ fontSize: 12, color: "var(--danger)" }}>Requires job description to generate.</p>}
